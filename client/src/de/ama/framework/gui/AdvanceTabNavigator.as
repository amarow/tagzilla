...
import mx.managers.DragManager;
	 ...
	 */
  	public class AdvanceTabNavigator extends TabNavigator	{
  		...
  		[Embed(source="/de/ama/framework/gui/assets/WindowCloseButton.gif")]
		private static const WINDOW_CLOSE_BUTTON_1:Class;
		[Embed(source="/de/ama/framework/gui/assets/WindowCloseButton2.gif")]
		...
		protected function addCloseButton(event:MouseEvent):void{
			...
			if(event.target is Tab && !DragManager.isDragging){
				if(event.target is Tab && tabBar.contains(Tab(event.target))){
        ...
        protected function doDragOver(event:DragEvent):void {
        	if(tabBar.getChildIndex(DisplayObject(event.dragInitiator)) >= 0){
	        	...
	        	if(targetTabIndex >= 0){
	        		event.dragSource.addData(targetTabIndex, TARGET_TAB_INDEX);
	        	}
        	}
        }
        
        protected function doDragDrop(event:DragEvent):void {
			if(tabBar.getChildIndex(DisplayObject(event.dragInitiator)) >= 0 && event.dragSource.hasFormat(TARGET_TAB_INDEX)){
	        	tabBar.graphics.clear();
				var sourceTabIndex:Number = tabBar.getChildIndex(DisplayObject(event.dragInitiator));
				var targetTabIndex:Number = Number(event.dragSource.dataForFormat(TARGET_TAB_INDEX));
